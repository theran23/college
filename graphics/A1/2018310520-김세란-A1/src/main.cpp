...
static const uint	MIN_CIRCLE = 2;		// minimum number of circle
static const uint	MAX_CIRCLE = 512;		// maximum number of circle
uint				NUM_TESS = 256;
uint NUM_CIRCLE = 20; // initial number of circle
double E = 0.0f;
...
void update()
{
	...
	// update circle number by the pressed keys 
...
void render()
{
	...
	for (auto& c : circles)
	{
		c.calculate(circles, delta_t);
	}

	//double energy1 = 0.0f;
	//for (auto& c : circles)
	//{
	//	energy1 += c.radius * c.radius * distance(c.velocity, vec2(0.0f, 0.0f)) * distance(c.velocity, vec2(0.0f, 0.0f));

	//}
	for (auto& c : circles)
	{
		c.velocity += c.delta_v;
	}

	//double energy2 = 0.0f;
	//for (auto& c : circles)
	//{
	//	energy2 += c.radius * c.radius * distance(c.velocity, vec2(0.0f, 0.0f)) * distance(c.velocity, vec2(0.0f, 0.0f));
...
void update_circle_num()
{
	...
	if (b.add) NUM_CIRCLE++; if (b.sub) NUM_CIRCLE--;
	if (NUM_CIRCLE<MIN_CIRCLE || NUM_CIRCLE>MAX_CIRCLE) return;

	circles = std::move(create_circles(NUM_CIRCLE));

	printf("> number of circles = % -4d\r", NUM_CIRCLE);
}
...
void mouse(GLFWwindow* window, int button, int action, int mods)
{
	if (button == GLFW_MOUSE_BUTTON_LEFT && action == GLFW_PRESS)
	{
		...
		vec2 clicked = vec2(float((2*pos.x/window_size.x) -1)*(3/float(2)), float(-(2 * pos.y / window_size.y) + 1));

		for (auto & c: circles) {
			if (distance(c.center, clicked) < c.radius) {
				printf("a circle is clicked\n");
				c.is_clicked = true;
				c.velocity = vec2(0);
				c.delta_v = vec2(0);
			}
...
int main(int argc, char* argv[])
{
	...
	{
		if (frame == 0) { t = float(glfwGetTime()); delta_t = 0; }
		else { float tmp_t = float(glfwGetTime()); delta_t = tmp_t - t; t = tmp_t; }
