...
#else
	char path[PATH_MAX]={}, dir[PATH_MAX]={}, fname[PATH_MAX]={}, ext[PATH_MAX]={};
	module_path_t()
	{
		char link[PATH_MAX]; sprintf(link,"/proc/%d/exe",getpid()); readlink(link,path,PATH_MAX); // read module path
		char *slash=path,*next; while((next=strpbrk(slash+1,"/\\"))) slash=next;
		strcpy(fname,path==slash?path:slash+1); if(path!=slash) strncpy(dir,path,slash-path+1);
		char* dot=strrchr(fname,'.'); if(dot){strcpy(ext,dot);*dot='\0';}
	}
#endif
};
...
inline const char* absolute_path( const char* file_path )
{
	...
	char t[_MAX_PATH]; module_path_t m; sprintf_s( t, "%s%s%s", m.drive, m.dir, file_path ); // build absolute path
	for(auto& c:t) if(c=='/') c='\\'; // slash to backslash in Windows
	char f[_MAX_PATH]; _fullpath( f, t, _MAX_PATH ); // canonicalize the path
#else
	if(*file_path=='/') return file_path; // is already absolute path
	char f[PATH_MAX]; module_path_t m; sprintf( f, "%s%s", m.dir, file_path ); // build absolute path
	for(auto& c:f) if(c=='\\') c='/'; // backslash to slash in Linux
#endif
	return strdup(f); // return duplication
}
...
inline const char* cg_conf_path()
{
	...
	if(!conf_path[0])
	{
		...
		if(access(conf_dir,0)!=0) _mkdir( conf_dir );
...
inline GLFWwindow* cg_create_window( const char* name, int& width, int& height, int version_major=gl_version_t::major_default, int version_minor=gl_version_t::minor_default, bool show_window=true )
{
	...
	// initialization
	if(!cg_glfw_init_once()){ printf( "%s(): failed in glfwInit()\n", __func__ ); return nullptr; }
